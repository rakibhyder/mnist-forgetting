...
val_loss_part=[]
l=0
test_acc_part=np.zeros((num_classes,epochs_part*(num_classes-1)*num_classes))
for epoch in range(0,epochs_part):
        for num in range (0,num_classes):
                ...
                for i in range (0,num_classes):
                        ...
                        batch=class_len[i]
                        if i!=num:
                                hist=model.fit(x_train[k:k+batch], y_train[k:k+batch],batch_size=batch,epochs=1,verbose=1,validation_data=(x_test, y_test), shuffle=True)
...
#print(test_acc)
np.savez('mnist_result_sorted_gradual'+str(epochs),test_acc_full=test_acc_full,train_acc_full=train_acc_full,train_loss_full=train_loss_full,val_acc_full=val_acc_full,val_loss_full=val_loss_full,test_acc_part=test_acc_part,train_acc_part=train_acc_part,train_loss_part=train_loss_part,val_acc_part=val_acc_part,val_loss_part=val_loss_part)
